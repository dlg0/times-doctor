...
class LSTParser:
    ...
    GAMS_HEADER_RE = re.compile(
        r'^GAMS\s+[\d.]+\s+.*?Page\s+(\d+)',
        re.MULTILINE
    )
    
    TIMES_HEADER_RE = re.compile(
        r'^TIMES\s+--\s+VERSION\s+[\d.]+',
        re.MULTILINE
    )
    ...
    def parse(self) -> List[LSTSection]:
        ...
        # Read file
        with open(self.lst_path, 'r', encoding='utf-8', errors='replace') as f:
...
class ExecutionProcessor:
    ...
    def process(section: LSTSection) -> Dict:
        ...
        return {
            ...
            'text_summary': ExecutionProcessor._create_summary(
                total_time, peak_memory, major_ops
            )
        }
    ...
    def _create_summary(
        ...
        if major_ops:
            ...
            for i, op in enumerate(major_ops[:10], 1):
                lines.append(
                    f"  {i}. Line {op['line']}: {op['type']} {op['name']} "
                    f"({op['time']:.3f}s)"
                )
...
class ModelAnalysisProcessor:
    ...
    def process(section: LSTSection) -> Dict:
        ...
        for line in content.split('\n'):
            ...
            if match:
                ...
                cumul_time = float(match.group(3))
        ...
        return {
            ...
            'equations': equations,
            'text_summary': ModelAnalysisProcessor._create_summary(
    ...
    def _create_summary(total_equations: int, equations: List[Dict]) -> str:
        ...
        for i, eq in enumerate(equations[:10], 1):
            lines.append(
                f"  {i}. {eq['name']}: {eq['count']:,} equations "
                f"({eq['time']:.3f}s)"
            )
