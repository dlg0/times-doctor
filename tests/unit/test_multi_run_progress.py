...
class TestRunProgress:
    ...
    def test_format_details(self):
        run = RunProgress(name="test", mu=1.2e-4, primal_infeas=2.3e-2, dual_infeas=3.4e-3)
    ...
    def test_thread_safety(self):
        ...
        def update_run(run_name, iterations=100):
            for i in range(iterations):
                monitor.update_cplex_progress(
                    run_name, {"phase": "barrier", "iteration": str(i), "mu": 1e-2 / (i + 1)}
                )
