...
class TestPromptLoader:
    ...
    def test_load_placeholder_validation_fails_missing(self):
        ...
        with pytest.raises(ValueError, match="missing"):
            load_prompt_template("qa_check_compress", required_placeholders={"nonexistent"})
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            manifest = {
                "test_prompt": {
                    "current": "v1",
                    "versions": {
                        "v1": {"sha256": hashlib.sha256(b"Test {extra} content").hexdigest()}
                    },
                }
            }
    ...
    def test_checksum_warning_in_non_strict_mode(self, caplog):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            manifest = {
                "test_prompt": {"current": "v1", "versions": {"v1": {"sha256": "wrong_hash_value"}}}
            }
            (tmp_prompts / "manifest.json").write_text(json.dumps(manifest), encoding="utf-8")
    ...
    def test_checksum_strict_mode_fails(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            manifest = {
                "test_prompt": {
                    "current": "v1",
                    "versions": {"v1": {"sha256": "definitely_wrong_hash"}},
                }
            }
            (tmp_prompts / "manifest.json").write_text(json.dumps(manifest), encoding="utf-8")
...
class TestPromptBuilders:
    ...
    def test_build_llm_prompt_with_ranges(self):
        ...
        diagnostics = {
            "status": {},
            "ranges": {"rhs": (1e-6, 1e12), "bound": (0.0, 1e6), "matrix": (1e-8, 1e3)},
            "mixed_currency_files": [],
